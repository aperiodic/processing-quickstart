...
This is done by using the \textit{modulo} operator, which is represented by the \texttt{\%} symbol.
It goes in between two numbers, like \texttt{+}.
The value on its left is the one you want to have wrapped, and the one on its right is what you want to wrap around.
The right number is the biggest value that can come out of the operation.

This is similar to how clocks work.
To figure out what time it will be in five hours if it's ten right now, you add five to ten, and then wrap around twelve.
