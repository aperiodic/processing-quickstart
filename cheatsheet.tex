...
As you add more things to your sketches, it becomes unwieldy to keep track of what all the numbers mean, because numbers have no inherent meaning.
It also becomes harder to update, say, an often-used color, because you must find everywhere it's used and update it.

In order to solve these problems, we use \textit{variables}, which are a way of assigning a name to a value.
In order to use a variable, you must \textit{declare} it.
Declaring a variable has the general form of \texttt{type name = value}.
The \texttt{type} is what kind of variable it is, the \texttt{name} is whatever you want to call it, and \texttt{value} is what you want the name to represent.

Here is how you'd declare a \texttt{radius} variable with a value of 42:
\begin{lstlisting}
int radius = 42;
\end{lstlisting}
The \texttt{int} stands for \textit{integer}, which is another way of saying "whole number."

This declares a color variable named \texttt{olive} that looks olive:
\begin{lstlisting}
color olive = color(20, 77, 32);
\end{lstlisting}

After the prior two variables are declared, you can make an olive circle with
\begin{lstlisting}
fill(olive);
circle(100, 100, radius, radius);
\end{lstlisting}

\textbf{Exercise:} make variables for the circle's x and y coordinates.
